...
/** 關鍵字長度上限。放在 schema 外面是為了讓 400 的訊息跟驗證用同一個數字 */
const Q_MAX = 64
...
const PrizeQuery = PageQuery.extend({
   ...
   * 關鍵字搜尋。
   *
   * 為什麼一定要做在後端：卡冊是分批載入的，前端手上永遠只有已載入的那幾批。
   * 兩百張卡的人打開卡冊第一件事就是找一張卡，而前端過濾找得到的只有
   * 「剛好已經捲過的那些」—— 使用者搜「リザードン」得到 0 筆，卡卻躺在
   * 還沒載入的第 4 頁。這跟狀態分頁、排序、分組是同一個坑（見上面那段）。
   *
   * trim 之後空字串等於沒給：搜尋框清空、或使用者只打了幾個空白，
   * 都應該回到「整本卡冊」而不是變成一個永遠搜不到東西的條件。
   *
   * 長度上限 64：三個目標欄位裡最長的是卡名，站上最長的卡名也遠短於它；
   * 超過這個長度的輸入不會是「在找一張卡」，只會是貼錯東西或試探。
   * 不默默截斷 —— 截掉之後回一批對不上輸入的結果，比直接說不收更難懂。
   */
  q: z.string()
    .transform(s => s.trim())
    .refine(s => s.length <= Q_MAX, `關鍵字最多 ${Q_MAX} 個字`)
 ...
 * 把關鍵字包成 LIKE 樣式。
 *
 * `%` 與 `_` 是 LIKE 的萬用字元，使用者輸入裡的它們必須先跳脫，否則
 * 搜「100%」會變成「搜任何含有 100 的東西」，而搜「SV2a_025」會 match
 * 到 SV2a-025 以外的一堆卡 —— 使用者打的是他在卡上看到的字，不是樣式。
 * 反斜線本身也要跳脫（它是 LIKE 的預設跳脫字元），不然結尾的一個 `\`
 * 會把後面補上的 `%` 吃掉，變成語法錯誤。
 *
 * 回傳值一律當**參數**送進 SQL（postgres.js 的 ${} 是 bind parameter），
 * 不做字串拼接。
 */
...
prizes.get('/', async c => {
  ...
  if (!parsed.success) {
    ...
    const message =
      ...
      : which === 'group' ? '分組鍵不合法'
      : which === 'q' ? `關鍵字最多 ${Q_MAX} 個字，請縮短再搜一次`
      : '分頁參數不合法（limit 介於 1 到 100）'
    return c.json({ error: 'BAD_REQUEST', message }, 400)
  }
  const { limit, cursor, status, sort, group, q } = parsed.data
   ...
   * 所以：狀態過濾與關鍵字（那是「要看哪一批卡」，屬於統計範圍的一部分）
   * 留在這一層，游標（那是「我讀到哪」，是分頁的實作）擋在外面。
   *
   * 關鍵字擺這一層還有一個好處：三個比對欄位（卡名／鑑定編號／套牌代號）
   * 全都是 group_key 的來源欄位或它的同款屬性，所以「同一組要嘛整組命中、
   * 要嘛整組不中」—— 濾在窗之前不會把一組卡切一半，×N 仍然是使用者
   * 在這個搜尋結果裡真正持有的張數，而不是「這一頁剛好有幾張」。
   * 而且先濾再開窗，掃的列數只會更少。
   *
   ...
   * 真正貴的仍然是列表那邊的卡圖與 DOM。
   *
   * 關鍵字的成本評估（**刻意不加索引**）：contains 比對兩頭都有萬用字元，
   * 任何 btree 索引都用不到，一定是逐列取 jsonb 欄位再比。但被掃的列數
   * 不是全表，而是 prizes_user 限縮之後的「這個人的卡」—— 那個量級是
   * 幾百到幾千，跟這支端點本來就要開的窗同一個數量級，多出來的只是
   * 每列三次 ->> 與 ilike。要到「單一使用者手上幾萬列」才會有感，
   * 而那個規模真正該處理的是整支端點的全冊掃描，不是搜尋這一段。
   * 真要優化，正確的作法是 pg_trgm 的 GIN 索引（btree 對前置萬用字元
   * 無效），那要一支獨立遷移＋擴充套件，不該夾在一個查詢參數裡偷偷帶進來。
   */
  /**
   * 關鍵字條件。
   *
   * ---- 為什麼 q 不進游標，而換頁仍然對得上 ----
   * q 是**過濾條件**不是排序鍵：它決定「哪些列在結果集裡」，不決定
   * 「誰排誰前面」。游標的 payload 是排序鍵的值，多塞一段 q 進去只會
   * 讓段數浮動（而段數是 decodeCursor 的參數，見上面那段），
   * 卻換不到任何正確性 —— 同一組 q 下，比較式比的仍然是同一批鍵，
   * 第 2 頁接在第 1 頁最後一列之後，一列不漏也不重複。
   *
   * 換了關鍵字之後拿舊游標來換頁呢：前端換關鍵字時應該從頭載，但後端
   * 不能假設它一定會。這種情況下結果**仍然是良序的** —— 比較式對新的
   * 結果集照樣成立，使用者拿到的是「新關鍵字的結果裡、排在那個鍵之後的
   * 那一段」，可能少了前面幾筆，但不會出現重複列、跳格或亂序。
   * 之所以只能做到這樣：游標本身不記得自己是在哪個 q 下發出去的，
   * 要擋就得把 q 寫進 payload，而那等於把搜尋字串變成游標格式的一部分
   * （舊游標全部失效、段數又浮動一次）。代價與收益不成比例。
   *
   * ⚠️ 注意 dupes / value 兩種排序的鍵含 group_total / group_ref，
   * 而它們是在**套用 q 之後**的結果集上算的（見下面的 CTE）。所以換了 q
   * 之後舊游標指到的位置會偏移 —— 偏移不是錯亂（順序與唯一性都還在），
   * 但這正是前端換關鍵字要重新開始的理由。
   */
  const qLike = q ? likePattern(q) : null
  ...
  const book = sql`
    select p.id,
     ...
     where p.user_id = ${me}
       ...
       ${qLike ? sql`and (
              p.card->>'name' ilike ${qLike}
           or coalesce(nullif(btrim(p.cert_no), ''), p.card->>'certNo') ilike ${qLike}
           or p.card->>'setCode' ilike ${qLike}
         )` : sql``}
  `
