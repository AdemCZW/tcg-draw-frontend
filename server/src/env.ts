...
const Env = z.object({
  ...
  CORS_ORIGINS: z.string().default('http://localhost:5173'),
   /** 開發用端點。啟用時必須同時設 DEV_LOGIN_SECRET，避免單一旗標誤設就暴露接管端點。 */
   DEV_LOGIN: z.enum(['0', '1']).default('0'),
   DEV_LOGIN_SECRET: z.string().min(32, 'DEV_LOGIN_SECRET 至少要 32 個字元').optional(),
...
if (env.DEV_LOGIN === '1' && !env.DEV_LOGIN_SECRET) {
   console.error('[env] DEV_LOGIN=1 時必須設定 DEV_LOGIN_SECRET')
   process.exit(1)
}
