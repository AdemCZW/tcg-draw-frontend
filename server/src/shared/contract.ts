...
export type OrderError =
   ...
   * 掛單還在，但現在成立不了這筆交易。
   *
   * 目前唯一的成因是賣家的可動用點數押不出這筆訂單的保證金
   * （見 server/src/routes/orders.ts）—— 但**訊息刻意不說原因**：
   * 說了等於把賣家的財務狀態透露給任何一個按下購買的人。
   * 前端也照這個原則講：只說這張卡現在買不了、請稍後再試或看別的，
   * 不要試圖從這個碼推測賣家的狀況。賣家本人會收到講清楚原因的通知。
   */
  | 'LISTING_UNAVAILABLE'
