...
/* N 籤的池 = N 張實體卡（1 張鑑定卡 + N−1 張裸卡），全部先進卡冊再挑。
   一張實體卡一個籤位，所以裸卡那一項從 total: N−1 變成 N−1 列各 total: 1
   —— 籤數不變，測到的東西不變。 */
const makePool = async (tok: string, title: string, certNo: string, tickets = 4) => {
  const certId = await bookCard(tok, certCard(certNo))
  const plainIds: string[] = []
  for (let i = 0; i < tickets - 1; i++) {
    plainIds.push(await bookCard(tok, { ...plainCard, name: `${plainCard.name} ${uniqName()}` }))
  }
  return call(tok, '/v1/pools', {
