...
| # | 事情 | 驗證 | 現況 |
|---|---|---|---|
| D-1 | **寄存到期怎麼處理** | 實測（2026-09-06，`server/src/regress-autoship.ts`，68 passed / 0 failed） | **已拍板並實作：到期自動替買家申請出貨。**<br>到期那天 `sweepStashExpiry()`（`server/src/pools-service.ts`）建一張 `shipments`、把卡改成 `ship_requested`、呼叫既有的 `markShipRequested()` 掛上賣家的 72 小時出貨時鐘 —— 走的是買家自己按「申請出貨」時的**同一條路**。賣家不寄的後果因此完全沿用 F-5 那一套（`ship_default_at` / `markShipDefault()` / `sellers.default_count`），沒有另造一條平行的逾期路。<br>**只碰 `stashed`**：`listed` 會跟掛單打架（而且主人已經選了「賣掉」這個出口）、`in_pool` 是賣家自己押的卡沒有買家、`ship_requested` / `shipped` 已經在路上、`recycled` / `refunded` 已經不是他的卡、`in_book` 卡在自己手上。<br>**沒填收件地址的買家不建單**，只發一則「去補資料」（按人聚合、一個月一則）。建了的話賣家會在出貨頁上看到一筆**他無法完成**、逾期還會真的記他違約的義務 —— 那是平台自己製造出來的違約。補完資料之後下一輪掃描自動接手，使用者不用再按任何按鈕。<br>**冪等**靠「候選只看 `stashed`，而建單的同一個交易就把它改成 `ship_requested`」＋ `for update` 重鎖重判 ＋ `notify()` 的 refId。鎖序照全站紀律 prizes（`order by id` 整批）→ settlements → shipments，不碰 `sellers`。<br>**運費**：站上從來沒有運費機制（`shared/economics.ts` 只在文案裡提醒賣家運費會吃掉利潤），自動出貨走的是同一條路，所以沒有改變那個答案 —— 實體運費由賣家自行吸收。<br>**這一輪刻意不做「延長寄存」**：讓買家按一下就延 90 天，期限就又回到不代表任何後果。代價是還不想收貨的買家會被迫收貨；他的事前出口是到期前兩週的提醒（改成明講會自動出貨，並指向上架與買回價兩條路）。<br>**前端還沒跟上**：買家在卡冊上只看得到「出貨申請中」，看不到「這是系統自動申請的」也看不到寄去哪一份地址 —— 站上目前沒有任何買家端的 `shipments` 讀取端點。詳見同一輪的交付說明。 |
