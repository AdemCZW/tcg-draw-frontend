...
`--nav-total` 在手機上是 `--nav-h + --safe-b`，桌機是 0。

**唯一的來源是 `App.vue` 的 `.navClear`** —— 一塊高度等於 `--nav-total`、
接在頁尾後面的空白，跟著 `showChrome`（也就是底部導覽本身）出現與消失。

**規則：`--nav-total` 不可以出現在任何進入版面流的 `padding` / `margin` / `height` 裡**，
只能出現在 fixed / sticky 元素的定位算式。導覽是 fixed，能遮到的永遠只有文件的最末端，
所以讓位只需要一份；頁面容器再加一次，那一份下面還接著頁尾與 `.navClear`，
永遠沒有東西可以讓，只會變成使用者捲到底時多出來的一段黑。

歷史（為什麼這條規則要寫兩次）：`71beffb` 第一次收斂時，把唯一來源設成
「全域頁尾的 `padding-bottom`」。那個選擇有兩個弱點 —— 大廳沒有頁尾，
所以它得自己記得再寫一份；而頁面容器寫 `padding-bottom` 太自然，
沒有東西擋著下一個人又加一次 `var(--nav-total)`。`57d2cc6` 就是這樣在
`PlayPage` 上加了回來（實測 393×852 手機的空隙 162.5px、桌機只有 88px）。
改成 `.navClear` 之後，讓位跟「誰剛好是最後一個元素」脫鉤，頁面沒有理由再碰它。
...
- 市場交易手續費目前是 **0**。
- 過戶時**沒有**重設 `stash_expires_at`，買家繼承賣家剩下的寄存天數。**這是對的，不是漏掉**（2026-09-04 實測，見 open-issues D-2）：那 90 天量的是實體卡在 `custodian` 手上放了多久，而站內過戶不搬動實體卡；重設的話兩個帳號互相買賣就能無限延後到期提醒。買家看不到剩幾天才是問題，已補上揭露（`GET /v1/orders/listings/:id/stash` + 市場卡片頁）。到期之後要做什麼仍是 D-1，使用者還沒表態。
