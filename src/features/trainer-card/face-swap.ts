...
/**
 * 每個 session 最多幾次**使用者主動**的生成：**首次 + 重來 1 次**。
 *
 * 使用者拍板（2026-09-04）：「選擇自己要的生成一張，中途只能重新一次」。
 * 原本是 3 次 —— 那是規格 §9 給的數字，在「每人一次、不收點數」的定位下
 * 太寬：三次生成的期望成本是這一版的 1.5 倍，而第三次幾乎只發生在
 * 「使用者在挑臉」的情境，那不是這個功能要提供的東西。
 */
export const MAX_USER_GENERATIONS = 2

/**
 * 每個 session 最多幾次**實際打到上游**的呼叫。
 *
 * 規格 §9 只寫了「每 session 最多 N 次生成」，沒有定義自動重試算不算。
 * 沒定義的話，2 次生成 × (1 次 + 1 次自動重試) = 4 次付費呼叫，
 * 成本是預期的兩倍。這裡的決定是：
 *
 *   1. **自動重試不吃使用者的生成額度。** 模型沒回圖是我們這邊的問題，
 *      不該讓使用者少一次機會 —— 那會變成「越常壞、能試的次數越少」。
 *   2. **但整個 session 的上游呼叫另外有一道硬上限。**
 *
 * **這個上限刻意等於 MAX_USER_GENERATIONS。** 使用者拍板的是
 * 「一次生成 + 一次重來」，成本上限就是 2 次上游呼叫 ≈ NT$6.4
 * （單價見 docs/trainer-card-plan.md §4.2）。設成 3 的話，
 * 「一次自動重試」會偷偷把帳單再加 50%，而使用者從頭到尾只看到兩次生成。
 *
 * 代價講清楚：兩次都遇到 NO_IMAGE_RETURNED 的人，第二次的自動重試會撞到
 * 這道上限，看到的是「請稍後再試」而不是重試鈕。實測那個機率約 1/100
 * （規格 §8.1 說 NO_IMAGE_RETURNED 約 1/10）—— 用一個百分之一的壞體驗
 * 換一個「帳單絕不會超過講好的數字」的保證，這一版選後者。
 *
