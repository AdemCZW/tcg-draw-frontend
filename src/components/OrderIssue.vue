...
const props = defineProps<{
   ...
   * 這一筆現在還申訴得了嗎。
   *
   * 面板第一段要把這顆跟「我要申訴」分開，而分開的講法不能只有一種：
   * 已結案的訂單根本沒有那顆鈕（申訴只在進行中的訂單上），對著一個不存在的
   * 按鈕說「要退款請改用它」，等於把人推去找一個找不到的東西 ——
   * 而「已完成的訂單一樣掛得上」正是這個元件存在的理由之一（見檔頭）。
   *
   * 預設 true（照舊講法），呼叫端不傳也不會壞。
   */
  canDispute?: boolean
}>()
      ...
      <div v-if="open" class="oiWrap" @click.self="closeSheet()">
        ...
        >
          ...
          <p class="oiLead">
            ...
            <template v-if="canDispute !== false">
              要退款請改用訂單上的「我要申訴」—— 那條會凍結點數並要求完整未剪輯的開箱影片。
            </template>
            <template v-else>
              這一筆<b>已經結案，不能再申訴</b>了 —— 卡況或收貨的問題就從這裡講，
              客服會在工單裡處理。
            </template>
          </p>
