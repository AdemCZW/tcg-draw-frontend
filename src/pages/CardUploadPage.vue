 ...
 *   CERT_ALREADY_YOURS   已經在自己卡冊裡 —— 不是錯誤是提醒，
 *                        講清楚那張卡現在是什麼狀態
 */
...
/** CERT_ALREADY_YOURS：那句「現在是什麼狀態」的訊息。它是提醒不是錯誤 */
const yoursMsg = ref('')
...
const canSubmit = computed(() =>
  !!pick.value && !busy.value && (!graded.value || certNo.value.trim().length > 0))
...
async function submit() {
  ...
  try {
    ...
    } else if (e instanceof ApiError && e.code === 'CERT_ALREADY_YOURS') {
...
<template>
  <div class="container page upWrap">
    ...
    <!-- 第一步：挑卡。目錄挑身分；從卡冊挑到的是已經登記過的卡，
         送出會被擋下來並講清楚（那正是 CERT_ALREADY_YOURS 的意思） -->
