...
const rows = [
  ...
  /* 賣家設定。接在出貨後面：它是賣家身分的第三件事，而且是**上架的前提**
     （沒填聯絡方式上架會被擋）。放這裡而不是會員資料，是因為這一欄會被
     買家看到，跟那一頁的寄件電話剛好相反 —— 兩者混在一起，賣家會分不出
     哪一格是私的。 */
  { name: 'seller-settings', t: '賣家設定', icon: 'phone' },
...
const paths: Record<string, string> = {
  ...
  /* 手機外框。賣家設定那一格填的就是「買家打得到你的號碼」，
     用手機比用齒輪講得清楚 —— 齒輪在這張選單裡會被讀成「系統設定」 */
  phone: 'M7.5 2.8h9v18.4h-9zM10.6 5.6h2.8M12 18.4h.01'
}
</script>
